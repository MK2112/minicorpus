...
def training():
    ...
    minipile_train = load_dataset("parquet",
                                  data_files={
                                      "train": str(base_path / "MiniPile_DensityNano" / "minipile_DensityNano_train_shard_*.parquet"),
                                  },
                                  cache_dir=str(base_path / "MiniPile_DensityNano_Cache"),
                                  split="train")

    minipile_val = load_dataset("parquet",
                                data_files={
                                    "validation": str(base_path / "MiniPile_DensityNano" / "minipile_DensityNano_validation_shard_*.parquet"),
                                },
                                cache_dir=str(base_path / "MiniPile_DensityNano_Cache"),
                                split="validation")
    ...
    if os.path.exists(base_path / "minipile_DensityNano_train_tokenized"):
        minipile_train_tokenized = load_dataset("arrow", data_files=str(base_path / "minipile_DensityNano_train_tokenized/*.arrow"), split="train")
        minipile_val_tokenized = load_dataset("arrow", data_files=str(base_path / "minipile_DensityNano_val_tokenized/*.arrow"), split="train")
    else:
        ...
        minipile_train_tokenized.save_to_disk(base_path / "minipile_DensityNano_train_tokenized")
        minipile_val_tokenized.save_to_disk(base_path / "minipile_DensityNano_val_tokenized")
    ...
    output_dir = str(base_path / "pythia160m_minipile_DensityNano_trained")
    log_dir = str(base_path / "160m_minipile_DensityNano_logs")
    ...
    training_args = TrainingArguments(
        ...
        max_steps=828,                  # Adjusted for MiniPile (https://discuss.huggingface.co/t/how-does-max-steps-affect-the-number-of-samples-the-model-sees/69681)
        warmup_steps=int(0.01 * 828),   # 1% of total steps for warmup
        logging_dir=log_dir,
        logging_steps=10,
        eval_strategy="steps",
        eval_steps=100,     # Frequency for evaluation during training
        save_steps=828,     # Save at the end of training
    ...
    trainer.train()
    trainer.save_model(str(base_path / "pythia160m_minipile_DensityNano_trained")) # This saves the model weights
