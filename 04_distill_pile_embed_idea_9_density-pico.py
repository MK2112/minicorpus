...
class DistillConfig:
    ...
    cluster_info_path: Path = base_dir / "MiniPile_BatchKMeans/clustering_sorted/cluster_info_for_inspection.json"
    ...
    edition: str = "DensityPico" # Version of MiniPile, distinguishes file naming + output directory
    excluded_clusters: Set[int] = field(default_factory=lambda: {10, 15, 16, 22, 26, 28, 35, 37, 39, 40, 44, 46, 
                                                                 ...
                                                                 196, 200, 218, 219})
    train_count: int = 550_758
    val_count: int = 534
    test_count: int = 10_676 # Just correcting rounding errors: This produces 500000:500:10000
...
class MiniCorpusDistiller:
    ...
    def _shuffle_split(self, idxs_to_shuffle: List[int]) -> Dict[str, List[int]]:
        ...
        total_docs = len(idxs_to_shuffle)

        # Calculate target sizes that preserve the original proportions
        # but scaled to our actual document count
        target_total = self.config.train_count + self.config.val_count + self.config.test_count
        scale_factor = total_docs / target_total

        train_count = int(self.config.train_count * scale_factor)
        val_count = int(self.config.val_count * scale_factor)
        test_count = total_docs - train_count - val_count  # Ensure we use all documents
    ...
    def build_minicorpus(self):
        ...
        # Print the split size for each split
        for split_name, split_idxs in data_splits.items():
            print(f"\t{split_name.capitalize()} size: {len(split_idxs)}")
        del idxs_to_extract
...
# python 04_distill_pile_embed_idea_8_density-nano.py
# Detach from tmux session: Ctrl-b followed by d
# Reattach to tmux session: tmux attach -t mini_8
# tmux list-sessions
# tmux kill-session -t mini_8