...
def training():
    ...
    minipile_train = load_dataset("parquet",
                                  data_files={
                                      "train": str(base_path / "MiniPile_DensityTiny" / "minipile_DensityTiny_train_shard_*.parquet"),
                                  },
    ...
    if os.path.exists(base_path / "minipile_DensityTiny_train_tokenized"):
        minipile_train_tokenized = load_dataset("arrow", data_files=str(base_path / "minipile_DensityTiny_train_tokenized/*.arrow"), split="train")
        minipile_val_tokenized = load_dataset("arrow", data_files=str(base_path / "minipile_DensityTiny_val_tokenized/*.arrow"), split="train")
    else:
        ...
        minipile_train_tokenized.save_to_disk(base_path / "minipile_DensityTiny_train_tokenized")
        minipile_val_tokenized.save_to_disk(base_path / "minipile_DensityTiny_val_tokenized")
    ...
    output_dir = str(base_path / "pythia160m_minipile_DensityTiny_trained")
    log_dir = str(base_path / "160m_minipile_DensityTiny_logs")
    ...
    training_args = TrainingArguments(
        ...
        warmup_steps=int(0.01 * 1235),   # 1% of total steps for warmup
        logging_dir=log_dir,
        logging_steps=10,
        eval_strategy="steps",
