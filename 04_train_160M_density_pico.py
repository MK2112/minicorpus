...
def training():
    ...
    output_dir = str(base_path / "pythia160m_minipile_DensityPico_trained")
    log_dir = str(base_path / "160m_minipile_DensityPico_logs")
    ...
    training_args = TrainingArguments(
        ...
        max_steps=280,                  # Adjusted for MiniPile (https://discuss.huggingface.co/t/how-does-max-steps-affect-the-number-of-samples-the-model-sees/69681)
        warmup_steps=int(0.01 * 280),   # 1% of total steps for warmup
        logging_dir=log_dir,
        logging_steps=10,
        eval_strategy="steps",
        eval_steps=100,     # Frequency for evaluation during training
        save_steps=280,    # Save at the end of training
    ...
    trainer.train()
    trainer.save_model(str(base_path / "pythia160m_minipile_DensityPico_trained")) # This saves the model weights

if __name__ == "__main__":
    torch.cuda.empty_cache()
    training()

# tmux new -s 160m_minipile_density-pico
# conda activate minipile
# I ran with CUDA_VISIBLE_DEVICES=1 torchrun --nproc_per_node=1 04_train_160M_density_pico.py
# May need to reset in later run, I don't know
# Detach from tmux session: Ctrl-b followed by d
# Reattach to tmux session: tmux attach -t 160m_minipile_density-pico
# tmux list-sessions
# tmux kill-session -t 160m_minipile_density-pico
#
# 
