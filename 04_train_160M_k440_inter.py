...
def training():
    ...
    minipile_train = load_dataset("parquet",
                                  data_files={
                                      "train": str(base_path / "MiniPile_k440Inter" / "minipile_k440Inter_train_shard_*.parquet"),
                                  },
                                  cache_dir=str(base_path / "MiniPile_k440Inter_Cache"),
                                  split="train")

    minipile_val = load_dataset("parquet",
                                data_files={
                                    "validation": str(base_path / "MiniPile_k440Inter" / "minipile_k440Inter_validation_shard_*.parquet"),
                                },
                                cache_dir=str(base_path / "MiniPile_k440Inter_Cache"),
                                split="validation")
    ...
    if os.path.exists(base_path / "minipile_k440Inter_train_tokenized"):
        minipile_train_tokenized = load_dataset("arrow", data_files=str(base_path / "minipile_k440Inter_train_tokenized/*.arrow"), split="train")
        minipile_val_tokenized = load_dataset("arrow", data_files=str(base_path / "minipile_k440Inter_val_tokenized/*.arrow"), split="train")
    else:
        ...
        minipile_train_tokenized.save_to_disk(base_path / "minipile_k440Inter_train_tokenized")
        minipile_val_tokenized.save_to_disk(base_path / "minipile_k440Inter_val_tokenized")
    ...
    output_dir = str(base_path / "pythia160m_minipile_k440Inter_trained")
    log_dir = str(base_path / "160m_minipile_k440Inter_logs")
    ...
    trainer.save_model(str(base_path / "pythia160m_minipile_k440Inter_trained")) # This saves the model weights

if __name__ == "__main__":
    torch.cuda.empty_cache()
    training()

# tmux new -s 160m_minipile_k440_inter
# conda activate minipile
# I ran with CUDA_VISIBLE_DEVICES=1 torchrun --nproc_per_node=1 03_train_160M_k440_inter.py
# May need to reset in later run, I don't know
# Detach from tmux session: Ctrl-b followed by d
# Reattach to tmux session: tmux attach -t 160m_minipile_k440_inter
# tmux list-sessions
# tmux kill-session -t 160m_minipile_k440_inter
#
# 
