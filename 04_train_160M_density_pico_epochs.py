...
def training():
    ...
    output_dir = str(base_path / "pythia160m_minipile_Pico_Epoch_trained")
    log_dir = str(base_path / "160m_minipile_Pico_Epoch_logs")
    ...
    training_args = TrainingArguments(
        ...
        num_train_epochs=2,            # Since train_iters gets set, use num_train_epochs=1.5 like for The Pile
        ...
        warmup_steps=int(0.01 * 488),    # 1% of total steps for warmup
        logging_dir=log_dir,
        logging_steps=10,
        eval_strategy="steps",
    ...
    trainer.train()
    trainer.save_model(str(base_path / "pythia160m_minipile_Pico_Epoch_trained")) # This saves the model weights

if __name__ == "__main__":
    torch.cuda.empty_cache()
    training()

# tmux new -s 160m_minipile_pico_epoch
# conda activate minicorpus
# I ran with CUDA_VISIBLE_DEVICES=1 torchrun --nproc_per_node=1 04_train_160M_density_pico_epochs.py
# May need to reset in later run, I don't know
# Detach from tmux session: Ctrl-b followed by d
# Reattach to tmux session: tmux attach -t 160m_minipile_pico_epoch
# tmux list-sessions
# tmux kill-session -t 160m_minipile_pico_epoch
#
# 
